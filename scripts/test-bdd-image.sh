...
test_nix_source_build_uses_first_party_sccache() {
  ...
  assert_contains "$flake" 'postBuild ='
  assert_contains "$flake" 'sccache --show-stats --stats-format=json'
  assert_contains "$flake" 'bddCompilerStats = pkgs.runCommand'
  ...
  assert_contains "$SCRIPT" 'nix build --option sandbox false .#bddCompilerStats'
  assert_contains "$SCRIPT" 'cat "$(readlink -f /tmp/bdd-compiler-stats)"'
