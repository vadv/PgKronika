...
if [ "$1" = "manifest" ] && [ "$2" = "inspect" ]; then
  case "$3" in
    ...
    *seed-v1) [ "${MOCK_SEED_EXISTS:-0}" = "1" ] && exit 0 || exit 1 ;;
    *) exit 1 ;;
  esac
fi

if [ "$1" = "buildx" ] && [ "$2" = "imagetools" ] && [ "$3" = "inspect" ]; then
  if [ "${MOCK_SEED_EXISTS:-0}" = "1" ]; then
...
run_case() {
  ...
  export BDD_BUILDER_SEED_IMAGE="ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
  : > "$MOCK_DOCKER_LOG"
  "$@"
...
test_seed_digest_used_for_miss() {
  local log
  log=$(run_case seed-hit env \
  ...
  assert_contains "$log" "manifest inspect ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
  assert_contains "$log" "buildx imagetools inspect ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1 --format {{.Manifest.Digest}}"
...
test_seed_can_be_disabled() {
  local log
  log=$(run_case seed-disabled env \
    BDD_BUILDER_IMAGE="ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new" \
    BDD_BUILDER_PULL=1 \
    BDD_BUILDER_USE_SEED=0 \
    MOCK_SEED_EXISTS=1 \
    "$SCRIPT" build-builder)
  assert_not_contains "$log" "seed-v1"
...
test_push_updates_exact_but_not_seed_by_default() {
  local log
  log=$(run_case push-no-seed env \
    BDD_BUILDER_IMAGE="ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new" \
    BDD_BUILDER_PUSH=1 \
    "$SCRIPT" build-builder)
  assert_contains "$log" "push ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new"
  assert_not_contains "$log" "tag ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
}

test_exact_hit_updates_seed_when_enabled() {
  local log
  log=$(run_case exact-hit-seed-update env \
    BDD_BUILDER_IMAGE="ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-exact-hit" \
    BDD_BUILDER_PULL=1 \
    BDD_BUILDER_UPDATE_SEED=1 \
    "$SCRIPT" build-builder)
  assert_contains "$log" "pull ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-exact-hit"
  assert_contains "$log" "tag ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-exact-hit ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
  assert_contains "$log" "push ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
  assert_not_contains "$log" "buildx build"
}

test_seed_updates_only_when_enabled() {
  local log
  log=$(run_case seed-update env \
    BDD_BUILDER_IMAGE="ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new" \
    BDD_BUILDER_PUSH=1 \
    BDD_BUILDER_UPDATE_SEED=1 \
    "$SCRIPT" build-builder)
  assert_contains "$log" "push ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new"
  assert_contains "$log" "tag ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-new ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
  assert_contains "$log" "push ghcr.io/acme/pgkronika/pgkronika-bdd-builder:deps-linux-amd64-seed-v1"
}
