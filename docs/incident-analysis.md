...
Endpoint работает с `ClockRelation::Unknown`. `EpisodeRefV1.start_us` — позиция
начала аномального окна, а не время исходного события или снимка. Более ранний
episode не становится причиной, более поздний — следствием; равная Unix epoch
у разных источников также не доказывает синхронизацию часов.

Исключение — сохранённое ребро `pg_locks.blocked_by`, полученное из
`pg_blocking_pids`. Для конкретного снимка blocker публикуется как `lead`, waiter
