...
`expected_interval_count_reason` содержит машинную причину. При доказанном
периоде полнота равна usable-интервалам, делённым на ожидаемое число интервалов
incident-окна; значение намеренно не ограничивается единицей. Нельзя делать
вывод об отсутствии события по одному полю
