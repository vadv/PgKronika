...
Endpoint работает с `ClockRelation::SameDomain`. По принятой продуктовой
договорённости timestamp снимка считается временем наблюдаемого события, а
`EpisodeRefV1.start_us` — source of truth для упрощённой классификации внутри
инцидента. Findings самого раннего сигнала получают `lead`, самого позднего —
`downstream`, промежуточные остаются `coincident`. Если минимальный и
максимальный timestamp равны, все findings остаются `coincident`. Если один
экстремальный timestamp разделяют несколько сигналов, все они получают роль
этого экстремума. Это продуктовая модель порядка наблюдений, а не утверждение о
физической причинности.

Временная классификация — fallback только для `coincident`. Уже назначенные
`amplifier` и структурные роли lock-edge имеют приоритет и не перезаписываются.
Если запрошенная роль противоречит стороне участника lock-edge, finding остаётся
`coincident` и не переинтерпретируется по времени.
...
свободном CPU сама линза не назначает `amplifier` или структурную роль: она
эмитит `coincident`, после чего общий temporal fallback может классифицировать
экстремальный по времени сигнал.
