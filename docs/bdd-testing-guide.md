...
Команды запуска, требования Docker/Buildx и модель кэша CI описаны в
[testing.md](testing.md). Этот документ отвечает только за структуру сценариев,
правила oracle и требования к harness.
...
3. **Оракулом является PostgreSQL.** Сценарий называет тип сравнения: exact,
   transformed, subset, floor, ceiling, top-n или schema. Монотонный
...
7. **Имена секций доменные.** BDD-сценарий пишет registry section name или
   BDD alias: `pg_stat_wal.pg15_17`, `pg_stat_database.pg14_17`,
   `pg_locks.pg14_18`, `replication_instance`. Числовой `type_id` — внутренний
   идентификатор registry; он не должен появляться в пользовательском тексте
   сценария, кроме тестов, которые явно проверяют registry/type-id mapping.
   Для column-level проверок предпочтительна форма `section <name>.<column>`:
   `section pg_stat_wal.pg15_17.stats_reset matches ...`. Если oracle SQL
   проверяет конкретный raw probe, можно использовать field alias:
   `section pg_control_checkpoint.timeline_id matches ...`.
...
- **Floor oracle.** PostgreSQL query даёт нижнюю границу для накопительного
  счётчика; хотя бы одно значение секции должно быть `>=` этой границы.
  Подходит для счётчиков, которые могут вырасти между setup и snapshot.
- **Ceiling oracle.** PostgreSQL query даёт верхнюю границу; каждое ненулевое
  ...
  snapshot_time`.
- **Top-n oracle.** Сценарий проверяет выбранные строки, лимит, порядок,
...
- **Window step pair.** Для монотонного счётчика сценарий сначала захватывает
  floor до snapshot, затем читает ceiling после snapshot и проверяет
  `floor <= recorded <= ceiling`. Это не oracle kind и не пишется как
  `matches the window oracle`.
- **Schema oracle.** Проверяет codec/layout contract без live PostgreSQL state.
  Такой сценарий не считается behavioral BDD для метрики.

Oracle SQL не должен копировать collector SQL. Он должен проверять смысл через
независимый запрос к системному представлению, точечный catalog lookup или
явную проекцию состояния PostgreSQL.
...
Scenario: row-lock wait is captured as W -> H with the transactionid edge
  ...
  Then section pg_locks.pg14_18 has exactly one row for session "W":
    | blocked_by    | [H]           |
    | lock_locktype | transactionid |
    | lock_mode     | ShareLock     |
    | lock_granted  | false         |
  And section pg_locks.pg14_18 has a root row for session "H" with blocked_by = []
...
- **Снимок.** `When the collector snapshots the segment` только запускает
  ...
  шагах.
- **Ассерт строки.** `Then section <section-name> has [exactly one] row for
  session "X": <table>` декодирует секцию, находит строку по PID сессии `X`,
  сверяет именованные колонки и разрешает `[Name]` placeholders.
- **Ассерт по ключу.** Для метрик без PID используется явный key: `datid`,
  ...
  `.feature`.
- **Оракул.** `Then section <section-name>.<column> matches the <oracle-kind>
  oracle: <SQL docstring>` выполняет oracle SQL и сверяет результат с секцией
  по объявленному типу. Старая форма `section <section-name> <column>` остаётся
...
- **Сценарии на новом стандарте.** Уже переведены `collector`/
  ...
  PG14+ layout для `pg_locks.pg14_18`.
- **Отложено.** PG10-13 layout для `pg_locks.pg10_13` остаётся codec-level
  покрытием вне live matrix. Oracle kinds `top-n` и `schema` реализуются вместе
...
- [ ] указывать oracle kind: exact, transformed, subset, floor, ceiling, top-n
      или schema; для монотонных счётчиков — оконная пара
