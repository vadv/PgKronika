...
3. **Оракулом является PostgreSQL.** Сценарий называет тип сравнения: exact,
   transformed, subset, top-N/capped, window/tolerance, ceiling или schema-only.
...
- **Сценарии на новом стандарте.** Уже переведены `pg_stat_archiver`, smoke,
  ...
  `pg_stat_user_tables`/`pg_stat_user_indexes` и `connection_pool`.
- **Оставшиеся сценарии.** `collector`/`bgwriter-checkpointer` и
  `pg_stat_wal` всё ещё используют matrix-wide broad steps в `main.rs` и
  `.feature`. Их нужно перевести на этот стандарт или оставить в списке
  отложенных работ.
- **Отложено.** Locks (`1_011_001`) переводятся после появления типа в `main`.
  `top-n` и `schema` oracle kinds реализуются вместе с первым сценарием, который
  действительно требует эти сравнения.
...
- [ ] указывать oracle kind: exact, transformed, subset, top-N/capped,
      window/tolerance, ceiling или schema-only;
