...
| 2 | `raw-index-all-families` | `every_populated_canonical_block_matches_forced_raw_and_restart_warm`; `all_family_range_edges_use_half_open_ownership_and_one_left_halo`; `every_all_family_partition_reconciles_to_exact_compact_sealed_facts`; `oracle-profile`; `timeline[15..18]` | IMPLEMENTED |
| 3 | `partition-seal-invariance` | `every_all_family_partition_reconciles_to_exact_compact_sealed_facts`; `ten_thousand_random_partition_seal_and_merge_seeds_are_invariant` | IMPLEMENTED |
...
| 8 | `live-seal-identity` | `append_then_seal_keeps_one_coherent_event_set`; `public_event_identity_ignores_lineage_but_retains_content`; `every_all_family_partition_reconciles_to_exact_compact_sealed_facts`; `duplicate_segment_contents_do_not_invent_path_based_identity` | IMPLEMENTED |
...
| 11 | `trusted-floor-downsampling` | `trusted_floors_and_unknown_scores_survive_partition_merge_and_downsample`; `every_all_family_partition_reconciles_to_exact_compact_sealed_facts` | IMPLEMENTED |
