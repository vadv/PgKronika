...
| 4 | `ovf-fault-fallback` | `corrupt_sidecar_is_atomically_replaced_at_the_same_path`; `wrong_source_at_the_expected_name_is_rejected`; `oversized_candidate_is_rebuilt_and_atomically_replaced`; `publication_failure_returns_fresh_facts_then_serves_the_fallback`; `lifecycle[15..18]` | IMPLEMENTED |
| 5 | `source-damage-visible` | `scheduled_source_scrub_prevents_a_durable_fact_from_masking_damage`; `every_consumed_source_body_crc_failure_stays_a_source_error` | IMPLEMENTED |
