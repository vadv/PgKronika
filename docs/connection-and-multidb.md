...
дорогие по стоимости данные (размеры). Решения опираются на reftool (рабочий
...
PgKronika на `tokio-postgres` (async). reftool — на синхронном `postgres` 0.19;
...
(дефолт 10 мин, как reftool). На срабатывании:
...
4. **Per-table (класс B):** top-N по размеру (`ORDER BY ... LIMIT N`, дефолт N≈500
   как reftool) и **обязательно пропуск залоченных** таблиц — `pg_relation_size`
...
- **Retry на блокировке НЕ делаем.** Для read-only мониторинга это избыточно:
  ...
  `lock_timeout`+retry — для DDL/миграций, не для сбора). Отход от reftool здесь
