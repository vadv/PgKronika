...
Из проекта полностью удаляются прежнее поле идентичности и управляющая им
переменная окружения. Запрет охватывает код, схемы, тесты, документацию,
конфигурацию и CI.
...
Прежний контракт требовал ровно одно служебное значение во всём диапазоне.
...
инцидентов. Подготовка входа больше не строит множество служебных идентификаторов
и не возвращает ошибки их отсутствия или конфликта.

Обёртка служебной области удаляется. Entity joins ограничиваются существующими
typed identity, snapshot provenance и lifetime mapping. Отдельная проверка
описательной идентичности не выполняется.
...
`instance_metadata` удаляется из `PLAN_CONTEXT_SECTIONS`. Удаляются контекст,
conflict tracking этой секции и проверки:
...
OS cumulative counters сохраняются как обычные серии без заявленного boot-bound
reset family. Для одной серии используется стабильный внутренний epoch,
выведенный из `MetricSeriesId`; уменьшение значения остаётся reset, а известный
пропуск остаётся gap. Потери
...
связанные только с отсутствующей или конфликтующей служебной идентичностью. Из plan quality
...
6. Entity join tests не содержат служебной области и сохраняют typed/snapshot
   ограничения.
