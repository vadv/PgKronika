# Дизайн: `pg_stat_user_tables` (type `1_003`)

- **Дата:** 2026-06-29
- **Статус:** на ревью (brainstorming → spec)
- **type_id:** `1_003_001` (PG10-12) / `1_003_002` (PG13-15) / `1_003_003` (PG16-18)
...
В объёме:
- Новый type_id `1_003` с тремя версиями схемы.
...
При большом числе таблиц снимать все нельзя. reftool берёт top-N по трём осям
...
1. **Объём → top-N** (floor reftool): активность ∪ размер ∪ bloat, каждая ось
...
- **V1** (`1_003_001`, PG10-12): базовый набор без `n_ins_since_vacuum`,
  `n_tup_newpage_upd`, `last_seq_scan`, `last_idx_scan` — 41 колонка.
- **V2** (`1_003_002`, PG13-15): + `n_ins_since_vacuum` — 42 колонки.
- **V3** (`1_003_003`, PG16-18): + `n_tup_newpage_upd`, `last_seq_scan`,
  `last_idx_scan` — 45 колонок.

NULL вместо 0 для «никогда»/«неприменимо» (last_*, toast_* без TOAST, idx/tidx
без индексов) — улучшение над reftool, который всё COALESCE'ит в 0 и теряет
...
снимок; reftool делает двумя запросами с merge, его отдельный statio-top-N
...
Связь с codec-cap: секция типа `1_003` объединяет строки всех баз; жёсткий
...
- BDD `features/user_tables.feature`: матрица создаёт ≥2 базы с таблицами →
  ...
  `1_003_00x`, резолв StrId через словарь сегмента. Отдельный сценарий: таблица
...
- ① danger = все 5 (A-E); ② порог wraparound `0.8*afma`; ③ danger + 3 колонки
  в v1; ④ `candidate_reason` отложен; type_id `1_003`; statio слит; NULL-вместо-0.
