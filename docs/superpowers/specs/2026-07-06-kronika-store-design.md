...
открывается и отдаёт весь читаемый остаток. Это осознанный урок из rpglot: там
...
- **`open` с битым `.pgm` среди валидных → open УСПЕШЕН**, остальные сегменты
  отдаются (не BAIL на первом битом — прямой урок rpglot `build_index`);
...
затухание (ловушка rpglot: OS-рейты без dt-cap затухают через часовой разрыв).
