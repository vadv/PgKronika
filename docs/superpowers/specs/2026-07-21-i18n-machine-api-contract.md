...
```json
{
  ...
  "instance": "https://pgkronika.dev/problems/occurrences/0123456789abcdef0123456789abcdef"
}
```
...
- `instance` — абсолютный HTTPS URI occurrence. Суффикс — непрозрачный
  server-generated correlation id из 32 lowercase hex символов; он не содержит
  URI, query, credential, path, SQL или error chain.
...
`catalog.capabilities[].reason`, а также
`evidence[].coverage.source_period.expected_interval_count_reason` используют
этот реестр. Diff `nodata` values (`reset`, `gap`, `first_point`, `anomaly`,
`not_collected`) остаются bounded domain enum, а не prose.
