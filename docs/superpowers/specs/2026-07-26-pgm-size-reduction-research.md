# Физическое сокращение PGM: измерения и текущий контракт
...
Публичный формат и имя файла — PGM и `N.pgm`. Реализация обновляет единственные
writer, reader и внутренний физический контракт на месте, сохраняя неизменный
внутренний magic. В продукте остаются один PGM writer, один PGM reader и один
контракт.
...
- ограниченная по памяти реализация seal;
...
Запуск запечатал три возрастных сегмента и один восстановленный хвост.
Контейнер не был OOM-killed. После seal демонстрационный построитель OVF
завершился ошибкой, разобранной в разделе 8.
...
### 3.1. Один контракт на месте

PGM остаётся единственным основным container format и продолжает называться
PGM. Путь готового сегмента остаётся `N.pgm`.

Реализация согласованно обновляет:

- внутреннюю раскладку PGM;
- единственный PGM writer;
- единственный PGM reader;
- внутренние проверки writer, reader, journal и store.

Header magic остаётся неизменной непрозрачной внутренней константой, а
`format_version` — полем проверки целостности, не названием поколения
продукта. Публичные названия PGM, расширение `.pgm` и HTTP API `/v1` не
меняются. В реализации нет альтернативного writer, reader, обхода,
переключателя или runtime mode.
...
Production seal не должен читать целый журнал или все секции одного типа в
RAM.
...
- descriptors;
- decoded Arrow buffers;
- sort indices;
- fan-in heads;
- output batch;
- внутренний буфер Parquet row group;
- dictionary conflict state.

Рекомендуемый hard ceiling для первой реализации — 128 МиБ поверх
долгоживущего состояния collector. Переполнение возвращает типизированный
`SealError::WorkBudgetExceeded`, не приводит к неограниченному allocation и
...
1. создаёт новый `LocalDirSnapshot` над каталогом с одним canonical PGM;
2. требует один sealed unit, ноль warnings и ноль damages;
3. выполняет полный `pg_stat_activity` query без cursor;
4. требует ноль gaps и отсутствие `next_cursor`;
...
2. В том же каталоге создать уникальный temp через `create_new`.
3. Потоково записать magic, section bodies, catalog и tail index.
...
Повреждённый `active.parts` или PGM не является пустым store. Collector/web
останавливается с типизированной ошибкой целостности и сохраняет байты без
reset, truncate или автоматического удаления.
...
Следующий PR должен быть завершённым вертикальным изменением: writer и reader
обновляются вместе и всегда используют один физический контракт.

### 11.1. Код

1. `kronika-format`
   - обновить внутреннюю раскладку PGM in place;
   - сохранить внутренний magic неизменным;
   - синхронно обновить внутренний PGM part внутри `PGMP`;
   - физические invariants sealed catalog;
   - типизированные ошибки целостности;
   - один production path.
2. `kronika-registry`
...
4. `pg_kronika-collector`
   ...
   - повреждённый `active.parts` останавливает startup без reset/truncate;
   - метрики bytes, rows, sections, wall, CPU, budget high-water и failures.
5. `kronika-reader`
   - единственный reader PGM;
...
6. `kronika-store`
   - явная диагностика нарушения целостности;
...
3. Запечатать обновлённый `N.pgm`.
4. Проверить единственную текущую внутреннюю пару magic/version и
   single-section contract.
...
### 11.4. Crash и Linux filesystems
...
- повреждённый PGM или journal остаётся byte-identical.
...
- crate README описывают компактную раскладку PGM и memory bounds;
- английский и русский README владельцев синхронны;
- operator docs называют внутренний magic только непрозрачной константой;
...
- нет альтернативного режима или переключателя формата;
