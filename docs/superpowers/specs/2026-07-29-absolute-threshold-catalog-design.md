...
Статус: реализовано аналитическое ядро и расширенный каталог из 69 политик;
web-интеграция не выполнена.
...
Первый web consumer описан в
`2026-07-30-threshold-frame-integration-design.md`. Отдельный PR добавит
numeric bindings в projection catalog и bounded `GET /v1/frame/{view}` с
серверными classifications для 14 доказуемых per-cell политик. Остальные
`MetricId` получат явную причину отсрочки; похожая, но семантически
несовместимая колонка не считается binding.
