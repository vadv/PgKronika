...
Статус: реализованы аналитическое ядро, расширенный каталог из 69 политик и
первый web consumer: 14 bound / 55 deferred.

Документ уточняет контракт Класса 1 из исследования
`2026-07-29-anomaly-highlight-research.md`. Расширение до полного числового
состава и config-bound правил описано в
`2026-07-30-complete-numeric-threshold-catalog-design.md`. PR не подключает
результат к web API.
...
например `r/w_await` и `errors/drops`. Каталог разворачивает такие строки
в отдельные типизированные записи и реализует 69 логических метрик.

Самодостаточное ядро и каталог появились раньше consumer. Отдельный
integration PR добавил bounded `GET /v1/frame/{view}`, DTO и OpenAPI без
клиентской раскраски. Исчерпывающий web manifest связывает 14 доказуемых
per-cell метрик, а остальные 55 оставляет deferred с типизированной причиной.
...
- Реализовать 69 логических метрик из доменов CPU, памяти, PSI, cgroup,
  диска, сети, PostgreSQL activity, cache/checkpoints, tables/vacuum,
  statements/plans и replication.
...
└── catalog/
    ...
    ├── cgroup.rs
    ├── storage.rs
    ├── postgres_activity.rs
    ├── postgres_io.rs
    ├── postgres_statements.rs
...
- `Limit`: наблюдаемое значение и фактический динамический лимит;
...
| `WarningLimit` | Сравнивает наблюдение с переданным вызывающим лимитом и сохраняет фактическую границу. |
...
`CountPerMinute`, `BytesPerSecond` или `Bytes`. Для составного `Evidence`
единицы операндов определяет вариант политики.
...
## Базовый состав каталога

Таблицы ниже задают исходные 42 политики. Ещё 27 политик PostgreSQL перечислены
в `2026-07-30-complete-numeric-threshold-catalog-design.md`. `warn` и `crit`
сохраняют строгий или нестрогий оператор из исследования. `-` означает
отсутствие уровня.
...
- `CATALOG` содержит ровно 69 записей.
...
Модуль добавил Rust API и не изменил формат PGM/OVF. После подключения
`GET /v1/frame/{view}` изменять `MetricId`, строковые коды, binding operands или
семантику границ можно только с явным пересмотром projection contract.

## Критерии приёмки

- Публичный `threshold` API компилируется без новых зависимостей.
- Каталог содержит 69 записей из двух нормативных таблиц.
...
- README на английском и русском описывают одинаковый контракт analytics и
  границу ответственности первого web consumer.
- Все workspace gates проходят.

## Последующие работы

Первый web consumer реализован по
`2026-07-30-threshold-frame-integration-design.md`: numeric bindings находятся
в projection catalog, bounded `GET /v1/frame/{view}` возвращает серверные
classifications для 14 доказуемых per-cell политик, а остальные 55 `MetricId`
имеют явную причину отсрочки. Похожая, но семантически несовместимая колонка не
считается binding.

Категориальные правила остаются следующим независимым этапом. Production
frontend будет применять готовый `level`, не повторяя пороги и операторы.
