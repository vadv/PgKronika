...
`docs/type-registry/os.md`), но гап-анализ против зрелых систем
мониторинга и против инцидентной практики DBA показывает дыры, из-за
которых часть классических аварий по записи не разбирается:
...
Внешняя база сравнения — каталоги сбора четырёх семейств систем, по их
документации и исходникам:

- postgres_exporter v0.20 — 26 встроенных коллекторов
  (<https://github.com/prometheus-community/postgres_exporter/tree/master/collector>)
  и набор custom queries CrunchyData pgMonitor
  (<https://github.com/CrunchyData/pgmonitor>);
- pgwatch v3 — 74 определения метрик и 15 пресетов из bundled
  `metrics.yaml`
  (<https://github.com/cybertec-postgresql/pgwatch/tree/master/internal/metrics>);
- pganalyze collector — schema stats, vacuum simulator, EXPLAIN samples и
  9 категорий Log Insights (<https://pganalyze.com/docs>,
  <https://github.com/pganalyze/collector>);
- okmeter, mamonsu, pgmetrics/pgDash — по их документации метрик
  (<https://github.com/postgrespro/mamonsu/blob/master/documentation/metrics.md>,
  <https://pgmetrics.io/docs/>).

Версии появления и изменения схем представлений PostgreSQL сверены по
release notes и документации 9.6–18 (<https://www.postgresql.org/docs/>).

## Что уже сильнее аналогов

Контекст для калибровки: PSI, полный cgroup v2, дерево блокировок с
глубиной, per-table freeze horizon, учёт дыр в логе, provenance-секции
покрытия и ускорение activity до 1 секунды под нагрузкой — этого в таком
составе нет ни в одной из сравнивавшихся систем. Каталог ниже закрывает
точечные дыры, а не перестраивает сбор.

## Не цели

- Алертинг, health-оценки, рекомендации (advisor-функциональность
  pganalyze/pgDash) — PgKronika записывает улики, а не мнения.
...
- Хэширование всего каталога для change detection в стиле pgwatch —
  хронология DDL решается лог-линзой (см. T2-5).
...
Снимается у всех или почти всех сравнивавшихся систем либо закрывает
невосстановимый постфактум класс аварий. Новых зависимостей сверх уже
используемых не добавляется: каталожные представления и функции под
`pg_monitor`; T1-5 — лог-линза поверх уже тейлящегося stderr, T1-6 живёт
в существующей линзе `pg_stat_statements`.
...
«сколько осталось COPY/базовому бэкапу». pgmetrics снимает все шесть.
...
событие даёт разбор. У pganalyze это отдельный класс (L73).
...
pgMonitor держит отдельный `top_wal`-запрос.
...
skip-счётчики). Честная оговорка: из сравнивавшихся систем снимает один
pgmetrics, а startup process частично виден в activity по wait events —
...
`confl_*` с PG18; `stats_reset` per-row. Снимают pgwatch и pgmetrics.
...
вторую половину вопроса T1-1 «архивация стоит — почему». Идёт пакетом с
T2-4/T2-5 (общая механика лог-линз).
...
кормящая int4-колонку, переполняется задолго до maxvalue (слепое пятно
pgMonitor, здесь закрыто). База, snapshot_full, 3600 с. Права:
...
цикла. pgwatch и pgMonitor живут на той же оценке.
...
wait_sampling трактуются через историю extversion. pganalyze собирает.
...
времени запроса (CPU против диска) вместо вычитания blk-времён из
total_time (подход okmeter). Версионируется по версии расширения
...
в pg_store_plans (агрегаты). Основной механизм pganalyze.
...
границам транзакций», не секунда. Этого нет ни в одной сравнивавшейся
системе.
...
- `pg_database_size()` исключён из линзы database осознанно (стоимость
  на каталогах с большим числом файлов). Дешёвая альтернатива pgwatch —
  оценка по `pg_class.relpages` (`db_size_approx`); кандидат в колонку
  существующей линзы database.
- `pg_stat_progress_vacuum` (`1_012`): PG18 добавил `delay_time` и
  `track_cost_delay_timing` — существующей линзе нужна новая версия
  layout'а; мелкий PR в составе T1-4.
...
1. Tier 1: T1-1 (`pg_wal_storage`) → T1-6 (топы WAL/temp) →
   ...
   включая версию `1_012`) → T1-7 (xmin-квадрант) → T1-8 (колонки) →
   T1-9 (prefetch).
2. Tier 2: T2-1 → T2-2 → T2-3 → T2-4/T2-5/T2-6 (пакет лог-линз) →
...
секции с честной пометкой в snapshot_coverage, когда объект, настройка
или права не обнаружены.
