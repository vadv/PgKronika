...
Для version-gated поля тип означает тип SQL-колонки. Если поле становится
`Option<T>` только из-за версии, в DSL пишется `T`, например
`plans: i64 = "plans" if ...`; сгенерированный `read` обернет значение в
`Some`. Если сама SQL-колонка nullable, в DSL пишется `Option<T>`, например
`query_id: Option<i64> = "query_id" if ...`; сгенерированный `read` вернет
nullable значение без второго `Option`.
