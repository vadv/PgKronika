...
stats_reset)`; коллектор читает `stats_reset` в `1_020_001`. Если view
недоступна, читатель разрывает ряд по уменьшению счётчика, но не может
подтвердить reset метаданными.
