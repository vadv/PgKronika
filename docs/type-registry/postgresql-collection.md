...
Этот файл описывает действующее поведение `pg_kronika-collector`: выбор
соединения, ограничения SQL, права, планирование источников и формирование
готовых PGM-файлов. Схемы секций находятся в
[`postgresql.md`](postgresql.md), точные запросы и обработка результатов — в
[`crates/kronika-source-pg`](../../crates/kronika-source-pg) и
[`bins/pg_kronika-collector`](../../bins/pg_kronika-collector).
...
| PostgreSQL logs | файловый доступ к stderr-логу или доставка sidecar | `1_029_001` фиксирует `source_unavailable`/`unsupported_format`/`permission_denied`/`disabled`/`query_failed`; типизированные события читаются только из stderr, `csvlog` не поддерживается |
...
снимок которого выполнил чтение.

Перед каждым фактическим чтением планов коллектор получает `reset_metadata`
из основного подключения и из найденной базы расширения. `ts` этой реальной
строки передаётся параметром в SQL планов и становится общим временем строк
планов, `snapshot_coverage` и `collection_coverage`. После числовой и
текстовой фаз коллектор читает reset-контекст ещё раз. Изменение времени
старта postmaster, reset-маркера планов, версии расширения или
`compute_query_id` аннулирует весь снимок; ошибка чтения reset-контекста даёт
тот же результат. Такой снимок не записывается и повторяется не позднее чем
через 30 секунд. Метаданные экземпляра, если они due в том же окне, читаются
до планов.
...
Сброс статистики: у vadv-форка нет `pg_store_plans_info`, поэтому
`pg_store_plans_reset_at` в согласованной строке остаётся `NULL`. Непрерывность
конкретной записи подтверждается её `first_call`; уменьшение counter-колонок
всё равно разрывает ряд. Расширение relocatable: сбор предполагает, что схема
расширения входит в `search_path` подключений коллектора.
...
stats_reset)`; коллектор читает `stats_reset` в согласованную строку
`1_020_001`. Ошибка probe/read аннулирует соответствующий снимок планов. Если
view действительно отсутствует и probe корректно возвращает это состояние,
маркер остаётся `NULL`, а анализ upstream-планов закрывается как неполный,
поскольку reset подтвердить нельзя.
...
продолжает формировать остальные секции. Ошибка или timeout lock-query ведёт к
...
`reset_metadata` пишется, когда источник due по планировщику или тик
форсирован сигналом. Кроме того, каждое успешно прочитанное
`pg_store_plans`-состояние получает отдельную строку с точно тем же `ts`.
Плановое чтение метаданных выполняется после обычных статистических секций;
согласованная с планами строка читается непосредственно перед планами и
проверяется повторным чтением после них.
...
`current_setting(..., true)` вернет `NULL`. В строке, согласованной со снимком
планов, это значение повторно читается на самом подключении источника
`pg_store_plans`, а не наследуется от `pool.main()`.
...
Если форк не предоставляет `pg_store_plans_info`, корректный отрицательный
probe оставляет `pg_store_plans_reset_at = NULL`. Ошибка probe/read в обычной
плановой строке деградирует это поле; при согласованном чтении планов она
аннулирует весь снимок, а не выдаёт неизвестный reset за непрерывность.
...
При старте восстановленные окна сразу преобразуются в готовый сегмент с `ts`
самого раннего окна. Возраст открытого сегмента после рестарта отсчитывается
заново.

Коллектор завершает сегмент, когда сырой размер журнала достигает
...
немедленно завершает открытый сегмент и сохраняет контракт
«сигнал → snapshot → файл». После этого `instance_metadata`,
...
сначала завершает накопленный сегмент, затем дописывает окно в пустой
...
Создание готового файла объявляется строкой `sealed <путь> reason=<причина>`;
причины: `forced` (SIGUSR2), `tick` (режим «сегмент на тик»), `size`, `age`,
`journal-full`, `recovered` (создан при старте из восстановленного журнала).
