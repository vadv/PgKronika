...
`reset_metadata` пишется одной строкой в каждый сегмент. Сегмент сегодня
запечатывается одним снапшотом, поэтому чтение выполняется в каждом снапшоте,
после сбора статистических секций.
...
Версии расширений не читаются из `pg_extension` текущей базы: `pg_extension` —
покаталожное представление, а расширение может стоять в любой базе. Коллектор
берёт `extversion` из кэшей источников секций `1_002` и `1_003`/`1_004` — тех
соединений, где расширения были найдены и читаются. `NULL` в `ext_*_version`
означает «коллектор сейчас не собирает это расширение», а не «нигде не
установлено».
...
`instance_metadata` пишется одной строкой в каждый сегмент. Серверная половина
читается с главного соединения:
...
Строка пишется на каждый top-N источник, у которого в этом снимке потерялись
строки: `total > collected`, либо часть источника пропущена по таймауту или
другой ошибке. Источник, уложившийся в лимиты, строки не получает — пустая
секция означает «усечений не было».

- `1_013`/`1_014`: `total` — сумма `count(*)` по покрытым базам; выпавшая по
  ошибке база не попадает ни в `total`, ни в `collected`, но поднимает
  `reason` (`1` — таймаут, `3` — прочее). Ошибка самого `count(*)` занижает
  `total` и не выдумывает усечение.
- `1_002`: `total` — `count(*)` на соединении источника расширения.
