...
каждом тике» — в отличие от интервала, равного тику, он не гоняется с
джиттером таймера. Граф ожиданий локов
...
базовому. Ускоренный интервал, равный базовому или больше, отключает триггер;
`0` означает «каждый тик». Переходы темпа пишутся в stderr.
