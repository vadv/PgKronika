...
Сегмент живёт дольше одного тика: окна сборов копятся в журнале
`active.parts`, и файл печатается по размеру журнала до запаковки
(`KRONIKA_SEGMENT_MAX_BYTES`, как WAL-сегмент — по заполнению) либо по
возрасту (`KRONIKA_SEGMENT_MAX_AGE_S` — страховка тихого инстанса, аналог
`archive_timeout`). Файл называется по `ts` первого окна. SIGUSR2 печатает
сегмент немедленно — сигнальный контракт «снапшот → файл» сохранён. После
печати служебные секции (instance, reset, settings) снова due: каждый файл
самодостаточен.

## Планировщик

Коллектор тикает внутренним таймером (`KRONIKA_INTERVAL_S`, по умолчанию 5 с;
`0` выключает таймер — сбор только по сигналам). На каждом тике планировщик
отдаёт множество источников, чей интервал истёк; остальные не читаются и их
секции в этом сегменте отсутствуют. Первый тик после старта читает всё —
первый сегмент самодостаточен. SIGUSR2 — форсированный тик: читает все
...
`bins/pg_kronika-collector/src/main.rs`. Граф ожиданий локов
...
а не на следующем тике. Тик, в котором ни один источник не дал строк,
сегмента не печатает.
