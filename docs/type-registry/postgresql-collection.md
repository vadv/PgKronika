...
| PostgreSQL logs | файловый доступ к stderr-логу или доставка sidecar | `1_029_001` фиксирует `source_unavailable`/`unsupported_format`/`permission_denied`/`disabled`/`query_failed`; типизированные stderr-события ограничены checkpoint/slow-query/lifecycle |
