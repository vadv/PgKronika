...
Форки `pg_store_plans` имеют разные наборы колонок и разные способы получить
текст плана, поэтому им назначены разные семьи типов. `1_004_001` описывает
vadv 2.x; `1_003_001` зарезервирован для `ossc-db/pg_store_plans`. Форк
определяется пробой `to_regprocedure('pg_store_plans(boolean)')`: сигнатура с
`showtext` есть только у vadv.
...
разных запросов сливаются по форме. Для эксплуатации нужен
`compute_query_id = on`. `queryid`/`planid` валидны в пределах одного инстанса,
PostgreSQL major и версии расширения.
...
точечный `GRANT EXECUTE`.

BDD-образ включает vadv-форк для PostgreSQL 17 и 18 (rev закреплён в
`flake.nix`). PG15/16 не включают `pg_store_plans`, потому что оба форка
устанавливают одноимённые файлы расширения.
