...
`reset_metadata` пишется одной строкой в каждый сегмент. Текущий коллектор
запечатывает сегмент одним снапшотом, поэтому чтение выполняется в каждом
снапшоте после сбора статистических секций.
...
Интервалы источников — env-ручки с дефолтами из утверждённого плана таймингов
(см. env-блок в `bins/pg_kronika-collector/src/main.rs`). Граф ожиданий локов
