...
`(userid, dbid, queryid)`, а `dbid` различает базы. Поэтому одного запроса
достаточно для всех баз. Расширение может быть установлено не в основной базе:
коллектор пробует сначала `pool.main()`, затем соединения `per_db()`, и запускает
сбор на первом соединении с установленным расширением. Если расширения нет
нигде, секция не пишется, а коллектор логирует пропуск.
