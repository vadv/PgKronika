...
Два форка `pg_store_plans` — это две разные семьи типов: у них различаются
наборы колонок и путь получения текста плана. `1_004_001` (vadv 2.x)
реализован; `1_003_001` (ossc) зарезервирован. Форк различается пробой
`to_regprocedure('pg_store_plans(boolean)')`: сигнатура с `showtext` есть
только у vadv.

Сбор `1_004_001` (класс A с оговоркой): статистика instance-wide, но
SQL-объекты расширения существуют только в базе, где выполнен
`CREATE EXTENSION`; discovery идёт по `pool.main()` и всем per-db
подключениям, найденный источник кэшируется. Чтение двухшаговое и живёт на
собственном темпе (`KRONIKA_PG_PLANS_INTERVAL_S`, дефолт 300 секунд; пустой
результат повторяется через 30 секунд): секция появляется только в сегменте,
чей снапшот выполнил чтение, `ts` строк — момент чтения.
...
- Шаг 2: текст на строку —
  ...
  бюджет чтения `KRONIKA_PG_PLAN_TEXT_BUDGET` (дефолт 8 MiB); строки сверх
  бюджета пишутся с `plan = NULL`.

Ключ строки: `(dbid, userid, queryid_stat_statements, planid)`; отдельно
хранится внутренний `queryid` расширения (усечённый до 32 бит core query id) —
по нему диагностируются коллизии усечения. `queryid_stat_statements` — мост к
`1_002`; при `compute_query_id = off` ключ расширения вырождается и планы
разных запросов сливаются по форме — `compute_query_id = on` обязательное
предусловие эксплуатации. `queryid`/`planid` валидны в пределах одного
инстанса, major и версии расширения.

Прод-требования: `shared_preload_libraries = 'pg_stat_statements,
pg_store_plans'`; рекомендуется `track_io_timing = on` (иначе `blk_*_time`
равны 0, что неотличимо от настоящего нуля) и `pg_store_plans.track_planning`
для `*_plan_time`. Функции `pg_store_plans(boolean)` и
`pg_store_plans_get_plan(...)` в 2.1 исполнимы PUBLIC — тексты чужих планов
доступны любому пользователю; рекомендуется `REVOKE ... FROM PUBLIC` и
выделенная роль коллектора с точечным GRANT EXECUTE.

В BDD-образе vadv-форк собран против PostgreSQL 17 и 18 (пин rev в
flake.nix); PG15/16 остаются без расширения до ossc-раскладки — оба форка
ставят одноимённые файлы, один кластер несёт один форк.
