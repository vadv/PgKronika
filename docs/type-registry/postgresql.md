...
- источник задаётся `KRONIKA_LOG_PATH` или обнаруживается через
  `pg_current_logfile('stderr')`; при падении PostgreSQL collector продолжает
...
- лимиты: 4096 строк, 1 МБ данных и 50 мс за цикл, 64 КБ на физическую строку;
  backlog свыше 64 МБ пропускается до последнего 1 МБ;
...
переполнении словаря строка не срывает snapshot: недоступные текстовые поля
...
Одна строка представляет один `LOG:` report autovacuum или autoanalyze.
...
Одна строка представляет один `log_temp_files` record. `size_bytes` обязателен:
