...
`toplevel`. `dbid` различает базы, поэтому одного запроса к представлению из
любой базы с установленным расширением достаточно для статистики всех баз.
Коллектор выполняет этот запрос один раз, без обхода по базам.
