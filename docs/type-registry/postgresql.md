...
`blocked_by` пуст. `depth` — расстояние от корня (кратчайший путь); рёбра графа
читаются из `blocked_by`. `root_pid` указывает, к какому корню привязан данный
узел блокирующего компонента.
