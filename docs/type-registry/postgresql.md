...
| `1_003_001` | `pg_store_plans`, форк ossc | — | зарезервирован | — |
...
- `1_003_001` — `ossc-db/pg_store_plans`, зарезервирован (сбор не реализован);
- `1_004_001` — vadv-форк, реализован.

Схемы и способ получения текста плана различаются, поэтому используются разные
`type_id`. Форк определяется пробой сигнатуры `pg_store_plans(boolean)`.
...
фиксирует в логе (тип `1_027_001`).
