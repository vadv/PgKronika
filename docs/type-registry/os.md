...
`cgroup_path` — значение маппинга, а не часть ключа сортировки. Ключ остаётся
обычным entity-first: `(pid, starttime, ts)`.

Семантика `snapshot_full` выбрана осознанно, хотя маппинг меняется редко.
