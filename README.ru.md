...
- **Кэш производных фактов.** Изменять один корневой каталог кэша может только
  один независимо созданный `FactStore` или процесс; клоны `FactStore`
  разделяют право записи. Остальные экземпляры могут читать готовые факты.
  Сборщик мусора работает только внутри `overview/v1`, ничего не удаляет после
  неполного, ограниченного или неавторитетного сканирования и не касается
  исходных PGM, файлов блокировок, символьных ссылок и нераспознанных файлов.
  Ограничения по логическим байтам и числу файлов выключены, пока оператор их
  не задаст. Задержка повторной записи не блокирует чтение готовых фактов.
