...
///
/// An unknown path under `/v1/` returns a Problem Details 404 rather than the
/// shell, so a mistyped API route is not masked by the UI.
pub(crate) async fn static_handler(uri: Uri) -> Response {
    let path = uri.path();
    if path == "/v1" || path.starts_with("/v1/") {
        return ApiProblem::route_not_found().into_response();
    }
