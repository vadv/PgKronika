...
// Logical section names, as registered in `kronika-registry` (a lens joins on the
...
/// `PG-FREEZE-006` (`xid_wraparound_risk`): the frozen-XID or min-MXID age
/// climbing toward the wraparound budget. Reports a coincident finding when the
/// worst in-window age leaves less than a quarter of the `2^31` budget as
/// headroom, the zone where the vacuum failsafe forces aggressive freezing.
/// Gauge evidence caps at medium.
pub(crate) struct XidWraparoundRiskLens;

impl XidWraparoundRiskLens {
    const ID: &'static str = "PG-FREEZE-006";
    const INPUTS: &'static [SectionColumn] = &[
        SectionColumn {
            section: PG_STAT_DATABASE,
            column: "frozen_xid_age",
        },
        SectionColumn {
            section: PG_STAT_DATABASE,
            column: "min_mxid_age",
    ...
    /// The XID/MXID space before wraparound, `2^31` transactions.
    const WRAP_BUDGET: f64 = 2_147_483_648.0;
    /// Trigger once age has spent three quarters of the budget.
    const HEADROOM_FLOOR: f64 = 0.25;
}

impl Lens for XidWraparoundRiskLens {
    fn id(&self) -> &'static str {
        Self::ID
    }
    ...
    fn evaluate(
        ...
        sink.charge_points(cluster.members.len())?;
        for member in &cluster.members {
            if member.logical_section != PG_STAT_DATABASE {
                continue;
            }
            let column = match member.column {
                "frozen_xid_age" | "min_mxid_age" => member.column,
                _ => continue,
            };
            let Some(reading) = typed.gauge_max(
                PG_STAT_DATABASE,
                column,
                &member.identity,
                context.incident_start_us,
            ...
            let headroom = Self::WRAP_BUDGET - reading.value;
            if headroom > Self::WRAP_BUDGET * Self::HEADROOM_FLOOR {
                continue;
            }
            sink.emit(FindingDraft::new(
                Role::Coincident,
...
/// `PG-VACUUM-005` (`vacuum_backlog`): dead tuples outpacing cleanup. Reports an
/// amplifier when the worst in-window dead fraction
/// `n_dead_tup / (n_dead_tup + n_live_tup)` for a table is elevated. Gauge
/// evidence caps at medium.
pub(crate) struct VacuumBacklogLens;

impl VacuumBacklogLens {
    ...
    const MIN_SAMPLES: usize = 1;
    /// A fifth of a table's tuples being dead is a real cleanup backlog.
    const DEAD_FRACTION_FLOOR: f64 = 0.2;
}

impl Lens for VacuumBacklogLens {
    fn id(&self) -> &'static str {
        Self::ID
    }
    ...
    fn evaluate(
        ...
        sink.charge_points(cluster.members.len())?;
        for member in &cluster.members {
            if member.logical_section != PG_STAT_USER_TABLES || member.column != "n_dead_tup" {
                continue;
            }
            ...
            let total = pair.a + pair.b;
            if total <= 0.0 || pair.a / total < Self::DEAD_FRACTION_FLOOR {
                continue;
            }
            sink.emit(FindingDraft::new(
                Role::Amplifier,
...
/// `PG-ANALYZE-004` (`stale_statistics`): a table's row estimates drifting from
/// reality. Reports an amplifier when the worst in-window modified fraction
/// `n_mod_since_analyze / (n_mod_since_analyze + n_live_tup)` is elevated. It
/// measures the drift, not whether a plan actually changed (a cross-section
/// question); gauge evidence caps at medium.
...
impl StaleStatisticsLens {
    ...
    const MIN_SAMPLES: usize = 1;
    /// A fifth of the live rows modified since the last analyze skews estimates.
...
impl Lens for StaleStatisticsLens {
    ...
    fn evaluate(
        ...
        for member in &cluster.members {
            ...
            let total = pair.a + pair.b;
            if total <= 0.0 || pair.a / total < Self::MODIFIED_FRACTION_FLOOR {
                continue;
            }
            sink.emit(FindingDraft::new(
                Role::Amplifier,
...
/// `PG-CONN-014` (`connection_saturation`): a database approaching its own
/// connection limit. Reports an amplifier when the worst in-window
/// `numbackends / datconnlimit` is elevated. It reads the per-database
/// `datconnlimit`; a database with no limit (`-1`) is skipped, and the
/// cluster-wide `max_connections` ceiling needs `pg_settings` (a cross-section
/// join). Gauge evidence caps at medium.
...
impl ConnectionSaturationLens {
    ...
    const MIN_SAMPLES: usize = 1;
    /// Eighty percent of the per-database limit is close enough to bite.
...
impl Lens for ConnectionSaturationLens {
    ...
    fn evaluate(
        ...
        for member in &cluster.members {
            ...
            if pair.b <= 0.0 || pair.a / pair.b < Self::SATURATION_FLOOR {
                continue;
            }
            sink.emit(FindingDraft::new(
                Role::Amplifier,
...
/// `PG-REPL-015` (`replication_lag`): a standby's replay falling behind. Anchored
/// on an anomalous `replay_lag_us`, reports a coincident finding when the worst
/// in-window byte gap `sent_lsn - replay_lsn` exceeds the floor. Per-stage
/// breakdown (write/flush/replay) is left to a hardened pass; gauge evidence
/// caps at medium.
pub(crate) struct ReplicationLagLens;

impl ReplicationLagLens {
    const ID: &'static str = "PG-REPL-015";
    const INPUTS: &'static [SectionColumn] = &[
        SectionColumn {
            section: PG_STAT_REPLICATION,
    ...
    const MIN_SAMPLES: usize = 1;
    /// A hundred mebibytes of unreplayed WAL is a real lag.
    const LAG_BYTES_FLOOR: f64 = 104_857_600.0;
}

impl Lens for ReplicationLagLens {
    fn id(&self) -> &'static str {
        Self::ID
    }
    ...
    fn evaluate(
        ...
        sink.charge_points(cluster.members.len())?;
        for member in &cluster.members {
            if member.logical_section != PG_STAT_REPLICATION || member.column != "replay_lag_us" {
                continue;
            }
            let Some(gap) = typed.paired_gauge(
                PG_STAT_REPLICATION,
                &member.identity,
                ("sent_lsn", "replay_lsn"),
                context.incident_start_us,
            ...
            if gap.samples < Self::MIN_SAMPLES {
                continue;
            }
            if gap.a - gap.b < Self::LAG_BYTES_FLOOR {
                continue;
            }
            sink.emit(FindingDraft::new(
                Role::Coincident,
...
/// `PG-SLOT-016` (`slot_wal_retention`): a replication slot pinning WAL. Reports a
/// coincident finding when the worst in-window `retained_bytes` for a slot
/// exceeds the floor. Gauge evidence caps at medium.
pub(crate) struct SlotWalRetentionLens;

impl SlotWalRetentionLens {
    ...
    const MIN_SAMPLES: usize = 1;
    /// A gibibyte of WAL held by one slot is worth surfacing.
    const RETAINED_BYTES_FLOOR: f64 = 1_073_741_824.0;
}

impl Lens for SlotWalRetentionLens {
    fn id(&self) -> &'static str {
        Self::ID
    }
    ...
    fn evaluate(
        ...
        sink.charge_points(cluster.members.len())?;
        for member in &cluster.members {
            if member.logical_section != PG_REPLICATION_SLOTS || member.column != "retained_bytes" {
                continue;
            }
            let Some(reading) = typed.gauge_max(
            ...
            if reading.samples < Self::MIN_SAMPLES || reading.value < Self::RETAINED_BYTES_FLOOR {
                continue;
            }
            sink.emit(FindingDraft::new(
                Role::Coincident,
...
pub(crate) fn active_catalog() -> Vec<Box<dyn Lens>> {
    vec![
        ...
        Box::new(XidWraparoundRiskLens),
        Box::new(VacuumBacklogLens),
...
mod tests {
    ...
    #[test]
    fn xid_wraparound_over_the_budget_reports_a_medium_coincident() {
        // 1.7e9 of the 2^31 budget leaves under a quarter headroom.
        let typed = gauges(
            PG_STAT_DATABASE,
            &[("frozen_xid_age", &[1_700_000_000.0; 2])],
        );
        assert_eq!(
            run_lens(
                &XidWraparoundRiskLens,
                PG_STAT_DATABASE,
                "frozen_xid_age",
                &typed
            ),
    ...
    #[test]
    fn xid_wraparound_reports_for_the_multixact_age_too() {
        let typed = gauges(PG_STAT_DATABASE, &[("min_mxid_age", &[1_700_000_000.0; 2])]);
        assert_eq!(
            run_lens(
                &XidWraparoundRiskLens,
                PG_STAT_DATABASE,
    ...
    #[test]
    fn xid_wraparound_with_healthy_headroom_reports_nothing() {
        let typed = gauges(
            PG_STAT_DATABASE,
            &[("frozen_xid_age", &[1_000_000_000.0; 2])],
        );
        assert!(
            run_lens(
                &XidWraparoundRiskLens,
    ...
    #[test]
    fn xid_wraparound_on_empty_input_reports_nothing() {
        assert!(
            run_lens(
                &XidWraparoundRiskLens,
                PG_STAT_DATABASE,
    ...
    #[test]
    fn vacuum_backlog_above_the_dead_fraction_reports_a_medium_amplifier() {
        // 30 dead of 100 tuples = 0.3, above the 0.2 floor.
        let typed = gauges(
            PG_STAT_USER_TABLES,
            &[("n_dead_tup", &[30.0; 2]), ("n_live_tup", &[70.0; 2])],
        );
        assert_eq!(
            run_lens(
                &VacuumBacklogLens,
    ...
    #[test]
    fn vacuum_backlog_with_few_dead_tuples_reports_nothing() {
        let typed = gauges(
            PG_STAT_USER_TABLES,
            &[("n_dead_tup", &[5.0; 2]), ("n_live_tup", &[95.0; 2])],
        );
        assert!(
            run_lens(
                &VacuumBacklogLens,
    ...
    #[test]
    fn vacuum_backlog_on_empty_input_reports_nothing() {
        assert!(
            run_lens(
                &VacuumBacklogLens,
                PG_STAT_USER_TABLES,
    ...
    #[test]
    fn stale_statistics_above_the_modified_fraction_reports_a_medium_amplifier() {
        // 40 modified of 100 touched rows = 0.4, above the 0.2 floor.
        let typed = gauges(
            PG_STAT_USER_TABLES,
            &[
                ("n_mod_since_analyze", &[40.0; 2]),
                ("n_live_tup", &[60.0; 2]),
            ],
        );
        assert_eq!(
    ...
    #[test]
    fn stale_statistics_with_little_drift_reports_nothing() {
        let typed = gauges(
            PG_STAT_USER_TABLES,
            &[
                ("n_mod_since_analyze", &[5.0; 2]),
                ("n_live_tup", &[95.0; 2]),
            ],
        );
        assert!(
    ...
    #[test]
    fn stale_statistics_on_empty_input_reports_nothing() {
        assert!(
            run_lens(
                &StaleStatisticsLens,
                PG_STAT_USER_TABLES,
    ...
    #[test]
    fn connection_saturation_near_the_limit_reports_a_medium_amplifier() {
        // 90 of a 100-connection limit = 0.9, above the 0.8 floor.
        let typed = gauges(
            PG_STAT_DATABASE,
            &[("numbackends", &[90.0; 2]), ("datconnlimit", &[100.0; 2])],
        );
        assert_eq!(
            run_lens(
    ...
    #[test]
    fn connection_saturation_ignores_a_database_without_a_limit() {
        // datconnlimit = -1 means no per-database limit; the reading is skipped.
        let typed = gauges(
            PG_STAT_DATABASE,
            &[("numbackends", &[90.0; 2]), ("datconnlimit", &[-1.0; 2])],
        );
        assert!(
            run_lens(
                &ConnectionSaturationLens,
    ...
    #[test]
    fn connection_saturation_well_below_the_limit_reports_nothing() {
        let typed = gauges(
            PG_STAT_DATABASE,
            &[("numbackends", &[50.0; 2]), ("datconnlimit", &[100.0; 2])],
        );
        assert!(
            run_lens(
                &ConnectionSaturationLens,
    ...
    #[test]
    fn connection_saturation_on_empty_input_reports_nothing() {
        assert!(
            run_lens(
                &ConnectionSaturationLens,
                PG_STAT_DATABASE,
    ...
    #[test]
    fn replication_lag_above_the_byte_gap_reports_a_medium_coincident() {
        // 500 MB sent vs 100 MB replayed = a 400 MB gap, above the 100 MiB floor.
        let typed = gauges(
            PG_STAT_REPLICATION,
            &[
                ("sent_lsn", &[500_000_000.0; 2]),
                ("replay_lsn", &[100_000_000.0; 2]),
            ],
        );
        assert_eq!(
            run_lens(
    ...
    #[test]
    fn replication_lag_with_a_small_gap_reports_nothing() {
        let typed = gauges(
            PG_STAT_REPLICATION,
            &[("sent_lsn", &[100.0; 2]), ("replay_lsn", &[90.0; 2])],
        );
        assert!(
            run_lens(
                &ReplicationLagLens,
    ...
    #[test]
    fn replication_lag_on_empty_input_reports_nothing() {
        assert!(
            run_lens(
                &ReplicationLagLens,
                PG_STAT_REPLICATION,
    ...
    #[test]
    fn slot_wal_retention_above_the_floor_reports_a_medium_coincident() {
        let typed = gauges(
            PG_REPLICATION_SLOTS,
            &[("retained_bytes", &[2_000_000_000.0; 2])],
        );
        assert_eq!(
            run_lens(
    ...
    #[test]
    fn slot_wal_retention_below_the_floor_reports_nothing() {
        let typed = gauges(PG_REPLICATION_SLOTS, &[("retained_bytes", &[1_000.0; 2])]);
        assert!(
            run_lens(
                &SlotWalRetentionLens,
                PG_REPLICATION_SLOTS,
    ...
    #[test]
    fn slot_wal_retention_on_empty_input_reports_nothing() {
        assert!(
            run_lens(
                &SlotWalRetentionLens,
                PG_REPLICATION_SLOTS,
