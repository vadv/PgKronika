//! Typed metric evidence forwarded to lenses.
//!
//! Anomaly episodes only locate an incident; a finding needs the underlying
//! values. This holds the honest per-column diffs and gauge readings the reader
//! already folded, keyed by series, so a lens reports measured numbers and
//! excludes intervals the reader marked unusable (reset, gap, first point,
//! timestamp anomaly, or a disabled source).
...
impl CounterPoint {
    /// The interval delta as an `f64`, or `None` when the reader marked the
    /// interval unusable. Recovered as `rate * dt` so the domain core never
    /// depends on the reader's scalar kind.
    #[allow(
    ...
    fn delta(self) -> Option<f64> {
        match self.point {
            DiffPoint::Value {
                rate, dt_micros, ..
            } => Some(rate * (dt_micros as f64) / 1_000_000.0),
            DiffPoint::NoData { .. } => None,
        }
    }
}

/// One gauge column's finite readings for one series, in snapshot-time order.
struct GaugeTrack {
    points: Vec<(i64, f64)>,
}
...
mod tests {
    ...
    #[test]
    fn a_gauge_reading_round_trips() {
        let mut typed = TypedInputs::new();
        typed.insert_gauge("db", "numbackends", id(1), vec![(10, 42.0)]);
        let track = typed
            .gauges
            .get(&TrackKey {
                section: "db",
        ...
        assert_eq!(track.points, vec![(10, 42.0)]);
    }
}
