...
| `GET /v1/incidents` | `source`, `from`, `to`; необязательные `window`, `step`, `threshold`, `eps_rel`, `epsilon`, `max_cluster_span`, `section` | Объединяет близкие по времени аномальные эпизоды в кандидаты на инциденты. Для каждого кандидата возвращает интервал и метрики-участники, а для всего ответа — покрытие, качество данных и пропущенную из-за лимитов работу; выводов о первопричине пока нет. |
...
Ошибка имеет форму `{ "error": "code", "detail": "message" }`, когда нужен
detail. Неизвестная секция даёт `404`, неверные параметры — `400`, а превышение
ограничения на входные или материализуемые данные — `413`.
...
- `/v1/incidents` пока завершает работу после группировки. В ответе `complete`
  остаётся `false`, массив `findings` пуст, а diagnostic lens catalog имеет
  состояние dormant.
