...
запуске удаляет только распознанные временные файлы публикации PGM, оставшиеся
после сбоя. Временные файлы OVF и проверки записи overview остаются под
управлением владельца overview.
...
синхронизируются вместе. Журнал нулевой длины, файл без заголовка, другая
версия, оборванный или повреждённый журнал прерывает запуск и остаётся без
изменений. После успешной публикации PGM файл становится корректным
синхронизированным пустым журналом версии 1.

Структура корня закрыта. Файл `.pgm` или `.ovf` непосредственно в корне,
символьная ссылка, неизвестный объект, неверная дата или `SegmentId` в чужом
дне прерывает запуск. Это первая поддерживаемая структура в проекте до первого
релиза. Данные разработки и тестов, которые не соответствуют этому контракту,
следует создать заново.
