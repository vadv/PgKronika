...
PostgreSQL, Linux, cgroup и, при включении, журнал stderr, добавляет одно
ограниченное окно в `active.parts`, затем финализирует журнал в
...
| Переменная | По умолчанию | Назначение |
| --- | ---: | --- |
| `KRONIKA_PG_LOG_ENABLED` | `true`, только если задан `KRONIKA_LOG_PATH` | Включить источник. |
...
У tailer'а фиксированные пределы строк, байт, времени, backlog и выходных
событий. Ротация, усечение, бинарные строки, пропуск backlog и исчерпание
бюджета превращаются в типизированные gap-строки: частичное чтение не выдаётся
за полное.
