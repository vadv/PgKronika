...
содержит `stderr`, затем вызывает `pg_current_logfile('stderr')`. Относительный
путь разрешается относительно `SHOW data_directory` либо `KRONIKA_LOG_ROOT`,
если задано это переопределение.
