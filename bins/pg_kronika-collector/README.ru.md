...
| --- | ---: | --- |
| `KRONIKA_PG_MAX_TABLES` | `500` | Не более N таблиц на измерение и базу. Допустимо 1–546. |
| `KRONIKA_PG_MAX_INDEXES` | `500` | Не более N индексов на измерение и базу. Допустимо 1–819. |
| `KRONIKA_PG_MAX_STATEMENTS` | `500` | Не более N строк на каждой из двух осей; их объединение содержит не более 2N строк. Допустимо 1–32768. |
| `KRONIKA_PG_MAX_LOCK_ROWS` | `1000` | Предел ожидателей, рёбер и узлов в секции блокировок. |
...
| `KRONIKA_PG_MAX_QUERY_TEXT` | `5000` | Предел символов текста запроса `pg_stat_statements`, применяется сервером через `left()`. Допустимо 1–65536. |
