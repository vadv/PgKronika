...
///
/// A position belongs to an episode when it is `Evaluated` with
/// `|m| > threshold`; a below-threshold or not-evaluated position ends the
/// current episode. The peak is the position with the largest `|m|`.
#[must_use]
