...
//! counters above it restarted, so a rate across them is invalid.
//!
...
pub struct BgwriterCheckpointer {
    ...
    /// manual `pg_stat_reset_shared('bgwriter')` or a postmaster restart. It
    /// governs the rate validity of the `pg_stat_bgwriter` counters above.
    #[column(g)]
    ...
    /// PG17+ the checkpointer resets independently of the background writer.
    #[column(g)]
