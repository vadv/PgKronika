//! Type `1_006_001`: `pg_stat_bgwriter` + `pg_stat_checkpointer`.
//!
//! PG17 moved checkpoint counters to `pg_stat_checkpointer`; missing
//! version-specific columns are stored as `None`.
...
pub struct BgwriterCheckpointer {
    ...
    #[column(t)]
    pub ts: Ts,
    /// Scheduled checkpoints. Before PG17: `checkpoints_timed`; PG17+:
    /// `num_timed`, which also counts skipped checkpoints.
    #[column(c)]
    ...
    /// Checkpoint write time, ms. PG17+ includes restartpoints.
    #[column(c)]
    ...
    /// Buffers written by checkpoint/checkpointer. PG17+ includes restartpoints.
    #[column(c)]
    ...
    /// `pg_stat_bgwriter.stats_reset`: last reset of this view. Use it for
    /// background-writer counter rates on all versions and checkpoint counter
...
mod tests {
    ...
    fn typed_decode_rejects_a_schema_that_does_not_match_the_contract() {
        ...
        // Typed decode must reject partial schemas, not gather known columns.
