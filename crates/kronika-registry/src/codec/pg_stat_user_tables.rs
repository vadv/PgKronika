//! Type `1_013_001`..`1_013_003`: `pg_stat_user_tables`.
//!
//! Per-table statistics, one row per selected table per database. In PG 10-18
//! the column set only grows: `n_ins_since_vacuum` arrives in PG13, and
//! `n_tup_newpage_upd` plus the `last_seq_scan`/`last_idx_scan` timestamps in
//! PG16. The source maps those catalog layouts to three layout versions.
//!
...
#[cfg(test)]
mod tests {
    use super::{PgStatUserTablesV1, PgStatUserTablesV2, PgStatUserTablesV3};
