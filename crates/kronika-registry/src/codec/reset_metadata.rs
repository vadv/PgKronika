...
pub struct ResetMetadata {
    ...
    /// `pg_stat_bgwriter` reset; `None` if the view or field is absent.
    #[column(g)]
...
mod tests {
    ...
    fn pg15_row() -> ResetMetadata {
        ResetMetadata {
            // Pre-PG17 / pre-PG16 and no extensions: checkpointer and io resets
            // absent, ext versions and GUCs unknown.
    ...
    fn roundtrip_preserves_values_and_nulls() {
        ...
        let decoded = ResetMetadata::decode(bytes.into()).expect("decode");
