Feature: Collector reads pg_stat_wal
  Section pg_stat_wal.pg15_17 is the PG15-17 pg_stat_wal layout. PG18 uses section
  pg_stat_wal.pg18 after write/sync counters moved out of pg_stat_wal. Each scenario
  ...
  Scenario: PG15 seals the V1 pg_stat_wal layout
    ...
    And section pg_stat_wal.pg18 is absent from the segment
    And section pg_stat_wal.pg15_17 stats_reset matches the exact oracle:
      """
  ...
  Scenario: PG18 seals the V2 pg_stat_wal layout
    ...
    And section pg_stat_wal.pg15_17 is absent from the segment
    And section pg_stat_wal.pg18 stats_reset matches the exact oracle:
      """
      SELECT (EXTRACT(EPOCH FROM stats_reset) * 1000000)::bigint
      FROM pg_stat_wal
      """
