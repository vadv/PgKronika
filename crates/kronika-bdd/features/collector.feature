Feature: Collector reads bgwriter and checkpointer stats
  Section pg_stat_bgwriter+pg_stat_checkpointer stores pg_stat_bgwriter on every supported PostgreSQL
  ...
  Scenario: PG15 seals the pre-PG17 bgwriter/checkpointer layout
    ...
    Then section pg_stat_bgwriter+pg_stat_checkpointer uses the pre-PG17 bgwriter/checkpointer layout
    And section pg_stat_bgwriter+pg_stat_checkpointer bgwriter_stats_reset matches the exact oracle:
      """
  ...
  Scenario: PG17 seals the split bgwriter/checkpointer layout
    ...
    And section pg_stat_bgwriter+pg_stat_checkpointer bgwriter_stats_reset matches the exact oracle:
      ...
      FROM pg_stat_bgwriter
      """
    And section pg_stat_bgwriter+pg_stat_checkpointer checkpointer_stats_reset matches the exact oracle:
      """
      SELECT (EXTRACT(EPOCH FROM stats_reset) * 1000000)::bigint
