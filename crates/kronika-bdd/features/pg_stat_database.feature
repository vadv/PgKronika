Feature: Collector seals pg_stat_database rows with catalog fields and counter data
  ...
  Scenario: isolated database row carries catalog fields and captures inserted rows
    ...
    Then section pg_stat_database.pg14_17 has a row with datname = [scenario database]:
      | datallowconn  | true  |
      | datistemplate | false |
      | datconnlimit  | -1    |
    And section pg_stat_database.pg14_17 tup_inserted matches the floor oracle:
      """
      SELECT 10::bigint
      """
    And section pg_stat_database.pg14_17 datallowconn matches the subset oracle:
      """
      SELECT datallowconn FROM pg_database WHERE datname = current_database()
      """
    And section pg_stat_database.pg14_17 datistemplate matches the subset oracle:
      """
