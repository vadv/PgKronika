...
Feature: Source-scoped timeline facts stay coherent across supported PostgreSQL versions
  A time-local fixture with fixed PostgreSQL event semantics and the collected
  database metrics must produce one reconciled publication through collection,
  sealed storage, a same-stem sibling OVF, canonical fact extraction, factor
  coverage, and the HTTP timeline.
