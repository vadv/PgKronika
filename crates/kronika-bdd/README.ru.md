...
и подключается к ним через `tokio-postgres`. Она проверяет саму инфраструктуру,
прогоняет коллектор `source-pg` вживую против каждой версии и доводит бинарь
`pg_kronika-collector` до конца — до запечатанного сегмента.
...
`features/collector.feature` также доводит бинарь коллектора до конца:
...
`KRONIKA_COLLECTOR_BIN`) против кластера, ждёт строку `ready`, посылает
`SIGUSR2` и читает напечатанную строку `sealed <path>`. Затем она открывает этот
сегмент через `kronika-reader` и проверяет, что секция `1_006_001` декодируется
ровно в одну строку снимка, а временной диапазон сегмента закреплён за этим
снимком. Это весь цикл запись/чтение — сбор, запечатывание, чтение — в одной
проверке.
