...
Это не полный прогон для PostgreSQL 15-18.
...
собирает его локально. `build-runtime` собирает `image.tar`, загружает образ в
Docker и тегирует его как `pgkronika-bdd:<platform>-sha-<runtime-key>`, если не
задан `BDD_RUNTIME_IMAGE`. Если такой локальный образ уже есть, `build-runtime`
переиспользует его по умолчанию; задайте `BDD_RUNTIME_REUSE_LOCAL=0`, чтобы
...
Для локального запуска BDD по тегу используйте цель Makefile из корня
репозитория:
...
`TAGS` обязателен и передаётся в Cucumber как `--tags`. Цель использует тот же
Docker/Buildx-путь образа, что и полный запуск, и временный tarball
runtime-образа в `/tmp`, если не задан `BDD_OUTPUT_TAR`. Если `BDD_RUNTIME_IMAGE`
не задан, локальный образ тегируется по платформе и хэшу BDD runtime-входов,
поэтому изменения в Makefile, helper-скриптах и README не пересобирают BDD
образ. Нефильтрованный полный прогон остаётся через `./scripts/bdd-image.sh run`;
он использует тот же content-keyed тег образа по умолчанию.
...
- `bdd metadata` вычисляет ключ зависимостей, runtime-ключ и ссылки на образы;
- `bdd matrix` берёт из реестра или собирает content-keyed runtime-образ, затем
  запускает его.
...
диска. Если шаг сборки всё-таки запускается, он использует `build-runtime` с тем
же переиспользованием локального exact-образа, что и Makefile-путь.

PR из форка не публикуют образы в GHCR. Они используют те же разделённые входы
для dependency/runtime, но не могут обновлять package-cache в реестре
репозитория.

В этом репозитории нет закоммиченного GitLab CI-конфига. Тот же скрипт можно
использовать в GitLab CI:
